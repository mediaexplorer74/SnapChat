...
using SnapChat.Converters.SnapDotNet;
...
namespace SnapChat.Pages
{
	public sealed partial class MainPage
	{
		...
		private void ExpanderView_OnExpanded(object sender, bool e)
		{
			...
			// Update action text.
			var friend = (from f in ViewModel.Account.Friends 
						  where f.Username == expander.Tag select f).First();
		...
		private void FriendItem_OnPointerEntered(object sender, PointerRoutedEventArgs e)
		{
			...
			if (friend == null || friend.Stories == null 
				|| !friend.Stories.Any() 
				|| friend.Stories.All(s => s.IsExpired 
				|| friend.Stories.Any(story => !story.IsCached)))
				return;
		...
		private async void FriendItem_OnTapped(object sender, TappedRoutedEventArgs e)
		{
			...
			{
				if (itemExpander.Tag == friend.Username 
					&& friend.Stories.Any() && itemExpander.IsExpanded)
				{
